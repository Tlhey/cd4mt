...
def sample_from_diffusion(
    ...
    """Sample latent tensors from a trained diffusion model with Karras schedule."""
...
def _decode_latents_to_mix(ae: EncoderDecoder, latents: torch.Tensor, num_stems: int, latent_dim: int) -> list[torch.Tensor]:
    """Decode [B, S*C, L] latents to list of B mono mix waveforms (sum of stems)."""
    import numpy as np
...
@torch.no_grad()
def _evaluate_fad_on_val(diffusion: Diffusion,
                         ae: EncoderDecoder,
                         cfg: dict,
                         device: torch.device,
                         val_loader,
    ...
    """Generate a small batch of samples and compute FAD vs. reference val audio."""
    sr = cfg["data"]["params"]["preprocessing"]["audio"]["sampling_rate"]

    # Get first batch to infer latent length and collect references
    try:
        first_batch = next(iter(val_loader))
    except StopIteration:
        return float("nan")

    # Determine latent length by one encode
    if "waveform_stems" in first_batch:
        ex_lat = encode_stems(ae, first_batch["waveform_stems"][:1])
        latent_len = int(ex_lat.shape[-1])
    else:
        latent_len = 128

    # Collect reference mixes from val set
    refs: list[torch.Tensor] = []
    for batch in [first_batch] + [b for i, b in zip(range(num_eval - len(refs)), val_loader)]:
        if "waveform_stems" in batch:
            wav_stems = batch["waveform_stems"]  # [B, S, T] tensor
            if not isinstance(wav_stems, torch.Tensor):
                wav_stems = torch.tensor(wav_stems)
            mix = wav_stems.sum(dim=1)  # [B, T]
            for i in range(mix.shape[0]):
                refs.append(mix[i].detach().cpu().float())
        else:
            wav = batch["waveform"]  # [B, T]
            if not isinstance(wav, torch.Tensor):
                wav = torch.tensor(wav)
            for i in range(wav.shape[0]):
                refs.append(wav[i].detach().cpu().float())
    ...
    latents = sample_from_diffusion(diffusion, shape, device, num_steps=steps, sigma_min=sigma_min, sigma_max=sigma_max)
    gens = _decode_latents_to_mix(ae, latents, cfg["model"]["cae"]["num_stems"], latent_dim)
...
def train(cfg: dict, resume: str | None = None):
    ...
    # Support both 'grad_accumulation_steps' and the alias 'grad_accum_steps'
    grad_accum = cfg["train"].get("grad_accumulation_steps", cfg["train"].get("grad_accum_steps", 1))
    ...
    try:
        eff_bs = cfg['data']['params']['batch_size'] * int(grad_accum)
    ...
    while step < max_steps:
        ...
        for batch in train_loader:
            ...
            if step % grad_accum == 0:
                ...
                # Periodic FAD evaluation (lightweight). Uses val set as reference, random samples as generated.
                eval_every = int(cfg["train"].get("eval_every", 0))
                fad_cfg = cfg.get("fad", {})
                if eval_every and (opt_step % eval_every == 0) and fad_cfg.get("enabled", True):
                    try:
                        fad_val = _evaluate_fad_on_val(
                            diffusion=diffusion,
                            ae=ae,
                            cfg=cfg,
                            device=device,
                            ...
                            steps=int(fad_cfg.get("steps", 40)),
                            sigma_min=float(fad_cfg.get("sigma_min", 0.002)),
                            sigma_max=float(fad_cfg.get("sigma_max", 5.0)),
                        )
                        print(f"[Eval] FAD: {fad_val:.4f}")
                        if swanlab:
                            try:
                                swanlab.log({"FAD": fad_val, "step": step})
                            except Exception:
                                pass
                    except Exception as e:
                        print(f"[Eval] FAD failed: {e}")
