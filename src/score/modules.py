...
class ConvBlock1d(nn.Module):
    ...
    def forward(
        ...
        if exists(scale_shift):
            scale, shift = scale_shift   #scale、shift 一般形状是 (B, C, 1)
            x = x * (scale + 1) + shift  #如果 scale≈0、shift≈0，这步近似恒等映射：x * 1 + 0
        x = self.activation(x)
...
class MappingToScaleShift(nn.Module):
    ...
    def forward(self, mapping: Tensor) -> Tuple[Tensor, Tensor]: 
        scale_shift = self.to_scale_shift(mapping)       # mapping (B, features)
...
class LearnedPositionalEmbedding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = rearrange(x, "b -> b 1")                      # (B,) -> (B,1)
        freqs = x * rearrange(self.weights, "d -> 1 d") * 2 * pi  # (B,1)*(1,D)->(B,D)
        fouriered = torch.cat((freqs.sin(), freqs.cos()), dim=-1) # (B,2D)
        fouriered = torch.cat((x, fouriered), dim=-1)             # (B,1+2D)
        return fouriered
