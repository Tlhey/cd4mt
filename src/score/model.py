...
class EDMNetAdapter(nn.Module):
    """
    让 OpenAI UNetModel 适配 Diffusion 的接口:
      Diffusion 期望: net(x_in, c_noise, **kwargs)
      OpenAI UNet 期望: unet(x, timesteps, context=None, y=None)
    """
    def __init__(self, unet: nn.Module):
        super().__init__()
        self.unet = unet

    def forward(self, x, c_noise, **kwargs):
        # c_noise: [B]
        context = kwargs.get("context", None)
        y = kwargs.get("y", None)
        return self.unet(x, timesteps=c_noise, context=context, y=y)
