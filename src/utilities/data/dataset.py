...
class MultiSource_Slakh_Dataset(DS_10283_2325_Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Normalize after summation to prevent clipping and maintain [-1, 1] range
        max_val = np.max(np.abs(data_dict['waveform']))
        if max_val > 1.0:
            data_dict['waveform'] = data_dict['waveform'] / max_val
