...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        # Use 2D 1x1 conv for qkv/proj to match checkpoints trained with conv2d weights
    ...
    def _forward(self, x):
        b, c, *spatial = x.shape
        # keep spatial dims for conv2d qkv/proj, then flatten for attention core
        x_dtype = x.dtype
        x_norm = self.norm(x)
        # Ensure conv2d input/bias dtypes align (avoid half/float mismatch under AMP)
        conv_dtype = self.qkv.weight.dtype
        if x_norm.dtype != conv_dtype:
            x_norm = x_norm.to(conv_dtype)
        qkv_2d = self.qkv(x_norm).contiguous()            # [B, 3C, H, W]
        qkv = qkv_2d.reshape(b, 3 * c, -1).contiguous()   # [B, 3C, HW]
        h = self.attention(qkv).contiguous()              # [B, C, HW]
        h_2d = h.reshape(b, c, *spatial).contiguous()     # [B, C, H, W]
        # proj_out in conv_dtype
        if h_2d.dtype != conv_dtype:
            h_2d = h_2d.to(conv_dtype)
        h_2d = self.proj_out(h_2d).contiguous()
        # Residual in original dtype
        if h_2d.dtype != x_dtype:
            h_2d = h_2d.to(x_dtype)
        return (x + h_2d).contiguous()
